...
    */
  if ( guessCount == 0 || $("clue").textContent.indexOf("_") == -1 || guesses.indexOf(letter) != -1) {
    return;
  }
...
     */
    ...
    if (guesses.indexOf(letter) != -1) {   // letter has been guessed
      clueString += letter + " ";
    } else {                              // not guessed
      clueString += "_ ";
    }
   ...
   */
  if (guessCount == 0) {
    $("guesses").innerHTML = "You lose.";    // game over (loss)
  } else if(clueString.indexOf("_") == -1) {
    $("guesses").innerHTML = "You win!!";     // game over (win)
  } else {
